    ...
    Array.from(document.getElementsByClassName('settingsAuthAccountLogOut')).map((val) => {
        val.onclick = (e) => {
            ...
            if(Object.keys(ConfigManager.getAuthAccounts()).length === 1){
                isLastAccount = true
                setOverlayContent(
                    'Alerta<br>Esta es tu ultima cuenta',
                    'Para usar el launcehr neceistas una cuenta.<br><br>¿Estas seguro que quieres quitar la cuenta?',
                    'Estoy seguro',
                    'Cancelar'
                )
                setOverlayHandler(() => {
...
function populateVersionInformation(version, valueElement, titleElement, checkElement){
    ...
    if(isPrerelease(version)){
        ...
        titleElement.innerHTML = 'Version Estable'
...
function populateSettingsUpdateInformation(data){
    if(data != null){
        ...
        if(process.platform === 'darwin'){
            settingsUpdateButtonStatus('Descarga de Githuv<span style="font-size: 10px;color: gray;text-shadow: none !important;">Cierra el launcher despues de la actualizacion.</span>', false, () => {
        ...
        settingsUpdateTitle.innerHTML = 'Tienes la ultima version del launcher'
        settingsUpdateChangelogCont.style.display = 'none'
        populateVersionInformation(remote.app.getVersion(), settingsUpdateVersionValue, settingsUpdateVersionTitle, settingsUpdateVersionCheck)
        settingsUpdateButtonStatus('Mirar si hay actualizaciones', false, () => {
            if(!isDev){
                ipcRenderer.send('autoUpdateAction', 'checkForUpdate')
                settingsUpdateButtonStatus('Revisando actualizaciones...', true)
            }
        })
    }
}
